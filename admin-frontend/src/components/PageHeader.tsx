    ...
    return (
        <div className="mb-10">
            ...
            <div className="flex flex-col gap-2">
                {title && <h3 className="text-xl sm:text-2xl font-bold text-slate-800 italic">{title}</h3>}
                {subtitle && <p className="text-slate-500 font-medium text-base sm:text-lg leading-relaxed max-w-2xl">{subtitle}</p>}
                {!subtitle && welcome && <p className="text-slate-500 font-medium text-base sm:text-lg leading-relaxed">Here's what's happening with Anu's Touch today.</p>}
            </div>
